...
class Net():
    def __init__(self, input_shape):
        ...
        self.model.add(layers.Conv2D(3, 5, input_shape = input_shape, activation = 'relu'))
        # In our example, output from first Conv2D is 24 x 24 x 3.
        # For MaxPooling2D, default strides is equal to pool_size.  Batch and layers are assumed to match whatever comes in.
        self.model.add(layers.MaxPooling2D(pool_size = 2))
        # In our example, we are now at 12 x 12 x 3.
        self.model.add(layers.Conv2D(6, 3, activation = 'relu'))
        # In our example, we are now at 10 x 10 x 6.
        self.model.add(layers.MaxPooling2D(pool_size = 2))
        # In our example, we are now at 5 x 5 x 6.
        self.model.add(layers.Flatten())
        # Now, we flatten to one dimension, so we go to just length 150.
        self.model.add(layers.Dense(100, activation = 'relu'))
        # Now we're at length 100.
        self.model.add(layers.Dense(50, activation = 'relu'))
        # Now we're at length 50.
